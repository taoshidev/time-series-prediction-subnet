...
class TestScoring(TestBase):
	...
	def test_core_scoring_logic(self):
		# rmses
		scores = {0: 300, 1: 250, 2: 200, 3: 150, 4: 100, 5: 50}
		scaled_scores = Scoring.simple_scale_scores(scores)
		# scaled rmse
		self.assertEqual(scaled_scores, {0: 0.0, 1: 0.19999999999999996, 2: 0.4, 3: 0.6, 4: 0.8, 5: 1.0})

		sorted_scores = sorted(
			scaled_scores.items(), key=lambda x: x[1], reverse=True
		)

		self.assertEqual(sorted_scores, [(5, 1.0), (4, 0.8), (3, 0.6), (2, 0.4), (1, 0.19999999999999996), (0, 0.0)])

		weighed_scores = Scoring.weigh_miner_scores(sorted_scores)
		self.assertEqual(weighed_scores, [(5, 0.3333), (4, 0.2667), (3, 0.2), (2, 0.1333), (1, 0.0667), (0, 0.0)])

		# add the weighed_scores to the existing EMA based score using the
		# geometric mean of the difficulty of the predicted move

		# take the yeojohnson of the updated EMA based scores to set weights
