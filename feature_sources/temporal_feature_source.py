...
class TemporalFeatureSource(FeatureSource):
    ...
    def _get_year_term(time_ms) -> tuple[int, int]:
        ...
        return start_ms, end_ms
    @staticmethod
    def _get_ds(time_ms,sample_count,interval_ms,dtype=np.float32) :
        current_time_ms = time_ms
        samples = np.empty(shape=sample_count, dtype=np.float32)
        ...
        for feature_index, feature_id in enumerate(self.feature_ids):
            ...
            if feature_id == FeatureID.EPOCH_TIMESTAMP_MS:
                ...
                for i in range(sample_count):
                    samples[i] = current_time_ms
                    current_time_ms += interval_ms
            elif feature_id == FeatureID.DS:
                samples = self._get_ds(start_time_ms,sample_count,interval_ms=interval_ms,  dtype=dtype)
            else:
                offset = 0
                divisor = 0
                term_function = None
