...
class ValiUtils:
	...
	@staticmethod
	def get_vali_weights_json(vali_stream_id : str) -> Dict:
		# wrapping here to allow simpler error handling & original for other error handling
		try:
			vweights = ValiBkpUtils.get_vali_file(ValiBkpUtils.get_vali_weights_dir()
			                                      + ValiBkpUtils.get_vali_weights_file(vali_stream_id))
		except FileNotFoundError:
			return {}
		else:
	...
	@staticmethod
	def set_vali_weights_bkp(vali_weights: Dict, stream_id: str):
		...
		for file in all_files:
			try:
				...
				if TimeUtil.now_in_millis() > unpickled_df.end + TimeUtil.hours_in_millis(1):
					...
					vali_streams = [vali_stream.stream_id for vali_stream in ValiStream]
					for vali_stream in vali_streams:
						request_to_complete[unpickled_df.request_uuid].predictions[vali_stream] = {}
					# add per stream_id
					request_to_complete[unpickled_df.request_uuid].predictions[unpickled_df.stream_id][
						unpickled_df.miner_uid] = unpickled_unscaled_data_structure
					request_to_complete[unpickled_df.request_uuid].files.append(file)
	...
	def generate_standard_request():
		...
		vali_streams = [vali_stream.to_dict() for vali_stream in ValiStream]
		return ClientRequest(vali_streams=vali_streams)
